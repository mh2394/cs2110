/* NetId(s): mh2394, xxxnn. Time spent: h hours, mm minutes.
...
public class RepostTree {
	...
	public RepostTree insert(Person c, Person p) throws IllegalArgumentException {
		...
		if (p == null || c == null || this.contains(c) || !this.contains(p))
			throw new IllegalArgumentException("Can't insert child");
		getTree(p).children.add(new RepostTree(c));
		return this.getTree(c);
	}
	...
	public int size() {
		//TODO 2
		int size = 1;		//start at root
		for(RepostTree rt : this.children){
			size+= rt.size();
		}
		return size;
	}
	...
	public int depth(Person p) {
		//TODO 3
		if (!this.contains(p)) return -1;
		if (this.getRoot() == p) return 0;
		int depth= 0;
		for (RepostTree rt : this.children) {
			if (rt.getTree(p)!= null) depth+= 1 + rt.depth(p); 
		}
		return depth;
	}
	...
	public int widthAtDepth(int d) throws IllegalArgumentException {
		...
		if (d < 0) throw new IllegalArgumentException("Depth can not be less than 0");
		if (d == 0) return 1;
		int width= 0;
		for (RepostTree rt : this.children) {
	...
	public List<Person> getRepostRoute(Person c) {
		...
		List<Person> Route= new ArrayList<Person>();
		// Record the root node in each recursion call.
		Route.add(this.getRoot());
		for (RepostTree rt : this.children) {
			if (rt.contains(c)) {
				List<Person> SubRoute = new ArrayList<Person>();
				SubRoute= rt.getRepostRoute(c);
				// Record the branch satisfied.
				for (Person pp : SubRoute) {
					Route.add(pp);
				}
			}
		}
	...
	public Person getSharedAncestor(Person child1, Person child2) {
        ...
        if (child1 == null || child2 == null 
        		...
        		|| !this.contains(child2)) return null;
        List<Person> Route1= this.getRepostRoute(child1);
        List<Person> Route2= this.getRepostRoute(child2);
        int size1= Route1.size();
        int size2= Route2.size();
        int min= size1;
        Person pp= Route1.get(0);
        if (size1 > size2) min= size2;
        for (int i=0; i < min; i++) {
        	  if (Route1.get(i) == Route2.get(i)) pp= Route1.get(i);
        	  else return pp;
        }
		return pp;
	}
	...
	//Helper function.
	private boolean helper(RepostTree rt1, Set<RepostTree> rt2) {
		for(RepostTree rt : rt2) {
			if ((rt1.getRoot()).equals(rt.getRoot()) && rt1.equals(rt)) {
				return true;
			}
		}
		return false;
	...
	public boolean equals(Object ob) {
        ...
        RepostTree RT = (RepostTree) ob;
        if (RT == this) {
        	return true;
        }
        for (RepostTree rt : children) {
        	  boolean bl= helper(rt, RT.children);
        	  if (bl == false) {
        		  return false;
        	  }
        }
		if (RT.getRoot() == this.getRoot()) return true;
		return false;
